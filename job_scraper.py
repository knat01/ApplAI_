...
def get_job_link_from_resume(resume_text, api_key):
    ...
    try:
        ...
        job_search_url = response.choices[0].message['content'].strip()
...
def scrape_job_listings(job_search_url, api_key):
    ...
    Args:
        ...
        api_key (str): The OpenAI API key for ScrapeGraph-AI.
...
def scrape_jobs(resume_text, user_data):
    ...
    Args:
        resume_text (str): The extracted text from the user's resume.
        user_data (dict): A dictionary containing user preferences and API key.
